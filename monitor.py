...
def report(local_standings):
    if len(local_standings[HOSTILE]) + len(local_standings[NEUTRAL]) == 0:
        ...
        airtest.touch(Template("resource/location_tab.png", threshold=0.92, resolution=(1440, 1080)))
        airtest.touch((400, 600))  # current system

    airtest.touch(EMPTY_POSITION_IN_CHAT)
    airtest.sleep(1)
    airtest.touch(Template("resource/corp_chat.png", threshold=0.78, resolution=(1440, 1080)))
    airtest.touch(Template("resource/send_button.png", threshold=0.92, resolution=(1440, 1080)))
    airtest.touch(Template("resource/close_button.png", threshold=0.92, resolution=(1440, 1080)))
...
def report_discord(discord_client, local_standings):
    ...
    if hostile_count + neutral_count == 0:
        ...
        discord_client.send_message(message, image='tmp/screen_local.png')


def open_chat_local():
    airtest.touch((300, 1000))
    airtest.touch(Template("resource/menu.png", threshold=0.92, resolution=(1440, 1080)))
    airtest.touch(Template("resource/character_tab.png", threshold=0.92, resolution=(1440, 1080)))
    airtest.sleep(1)


def get_local_overview():
    screenshot_file_name = 'tmp/screen_overview.png'
    airtest.snapshot(screenshot_file_name)
    screen = cv2.imread(screenshot_file_name, cv2.IMREAD_UNCHANGED)

    local_tab_up = Template("resource/local_overview_top.png", threshold=0.88, resolution=(1440, 1080)).match_all_in(screen)
    local_tab_bottom = Template("resource/local_overview_bottom.png", threshold=0.92, resolution=(1440, 1080)).match_all_in(screen)
...
def identify_standing(name_image):
    ...
    elif (Template("resource/blue_plus.png", threshold=0.88, resolution=(w, h)).match_all_in(name_image) is not None or
          # Template("resource/green_star.png", threshold=0.88, resolution=(w, h)).match_in(name_image) is not None
          # matching on gray scale image. blue_star image works for green_star as well.
          Template("resource/blue_star.png", threshold=0.88, resolution=(w, h)).match_all_in(name_image) is not None):
        standing = FRIENDLY
    elif Template("resource/red_mark.png", threshold=0.82, resolution=(w, h)).match_all_in(name_image) is not None:
        standing = HOSTILE
    elif not (Template("resource/observer.png", threshold=0.92, resolution=(w, h)).match_in(name_image) or
              Template("resource/observer_name.png", threshold=0.86, resolution=(w, h)).match_in(name_image)):
        standing = NEUTRAL

    logger.debug(f'standing: {standing}')
    if logger.level <= logging.DEBUG:
        results = Template("resource/red_mark.png", threshold=0.80, resolution=(w, h)).match_all_in(name_image)
...
def identify_local_in_overview():
    ...
    if local_overview is None:
        ...
        divider_template = Template("resource/divider.png", threshold=0.76, resolution=(w, h))
...
def identify_local_in_chat():
    ...
    screenshot_file_name = 'tmp/screen_chat.png'
    airtest.snapshot(screenshot_file_name)
    screen = cv2.imread(screenshot_file_name, cv2.IMREAD_UNCHANGED)
    ...
    cv2.imwrite('tmp/screen_local.png', local)
